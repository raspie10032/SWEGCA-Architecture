...
from tinylm_slicer.mosaic_cognitive_slot_memory import (
    cognitive_slot_value,
    slot_tensor_hash,
)
from tinylm_slicer.mosaic_cognitive_slot_topology import cognitive_slot_topology
from tinylm_slicer.mosaic_synapse_arbiter import SingleWorldArbiter, SynapseProposal
from tinylm_slicer.mosaic_evidence_accumulator import (
    AccumulatorDecision,
    is_authoritative_accumulator_decision,
)
from tinylm_slicer.mosaic_evidence_revision import (
...
class WorldWriteGates:
    ...
    _proposal_binding_digest: str = field(
        default="",
        repr=False,
        compare=False,
        kw_only=True,
...
def world_write_gates_from_decision(
    ...
    *,
    ...
    """Mint a capability for one exact proposal under accepted decision evidence."""
    ...
    accepted_addresses = frozenset(decision.evidence_addresses)
    proposal_addresses = tuple(
        address for batch in proposal.evidence_addresses for address in batch
    )
    if not decision.hypothesis_id or not accepted_addresses:
        raise PermissionError("World write decision has no bound evidence history")
    if proposal.hypothesis_id != decision.hypothesis_id:
        raise PermissionError("World write proposal hypothesis is not accepted")
    if not proposal_addresses or any(not address for address in proposal_addresses):
        raise PermissionError("World write proposal has no accepted evidence")
    if not set(proposal_addresses).issubset(accepted_addresses):
        raise PermissionError("World write proposal cites unaccepted evidence")
    proposal_binding_digest = _proposal_digest(proposal)
    gates = WorldWriteGates(
        ...
        _proposal_binding_digest=proposal_binding_digest,
    )
...
def _world_write_gate_digest(gates: WorldWriteGates) -> str:
    payload = {
        ...
        "proposal_binding_digest": gates._proposal_binding_digest,
    }
...
def _proposal_digest(proposal: SynapseProposal) -> str:
    payload = {
        "source": proposal.source,
        "hypothesis_id": proposal.hypothesis_id,
        "evidence_addresses": proposal.evidence_addresses,
        "delta_candidate": slot_tensor_hash(proposal.delta_candidate),
        "confidence": slot_tensor_hash(proposal.confidence),
        "contradiction": slot_tensor_hash(proposal.contradiction),
        "uncertainty": slot_tensor_hash(proposal.uncertainty),
        "target_slot_mask": slot_tensor_hash(proposal.target_slot_mask),
    }
    return hashlib.sha256(
        json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()
...
def bounded_world_write_receipt_from_dict(
    ...
    return BoundedWorldWriteReceipt(
        ...
        proposal_binding_digest=str(payload.get("proposal_binding_digest", "")),
    )
...
def bounded_verification_write(
    ...
    *,
    ...
    if gates._proposal_binding_digest != _proposal_digest(proposal):
        raise PermissionError("World write proposal does not match authority binding")
    if not commit:
        return BoundedWorldWriteResult(
